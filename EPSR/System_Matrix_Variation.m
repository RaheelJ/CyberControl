...
for VI=I
    ...
    eig_Ac=abs(eig(Acc));
    diff_Ac=diag(VI*eig_Ac);
    sysf.a=Acc+VI*diff_Ac;
    ...
    STime_1(rr)=settlingtime(df(14, :), tf, 30, 'MidPercentReferenceLevel', 90, 'StateLevels', [0, 0.5], 'Tolerance', 5)
    AN=50;
...
hold(axes6,'all');
% Create plot
plot(I,AVG_STime,'LineWidth',3,'Color',[1 0 0],'Parent',axes6,...
     'DisplayName','With Control');
plot(I,STime_1,'LineWidth',3,'Color',[0 0 1],'Parent',axes6,...
     'DisplayName','Without Control');
% Create legend
legend(axes6,'show');
% Create title
ylabel('Average Settling Time (sec)');
xlabel('Percentage Change in Eigenvalues of System Matrix A_{c} (%)');
title('Effect of Model Variations on System Settling Time','FontSize',20);
